...
// Thanks Philipp15b for the code :D
...
public class CompassTrackerUpdater implements Runnable {
	...
	public CompassTrackerUpdater(HungerCraft instance) {plugin = instance;}
	...
	private static long updateRate = 200;




	public static void setPlugin(HungerCraft pplugin) {
		plugin = pplugin;
	}
		...
		if(!isRunning()) {
			taskId = plugin.getServer()
				...
				.scheduleSyncRepeatingTask(
						plugin,
						new CompassTrackerUpdater(plugin),
						40L,
						updateRate
				);
			if(isRunning()) {
